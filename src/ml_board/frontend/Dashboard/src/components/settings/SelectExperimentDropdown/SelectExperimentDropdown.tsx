...
export default function SelectExperimentDropdown() {
    ...
    const [experimentIdsInDropdown, setExperimentIdsInDropdown] = useState([]);
        ...
        axios.get(rest_api_url + experiments_url).then((response) => {
            ...
            if(response.status === 200) {
                if (response.data.length > 0) {
                    let experimentIdsInDropdown:any = [];
                    response.data.map((exp_data:any) => {
                        experimentIdsInDropdown.push(exp_data.experiment_id);
                    });
            ...
            <Grid container spacing={{ xs: 2, sm: 2, md: 2, lg: 2 }}>
                <Grid item={true} xs={12} sm={12} md={12} lg={12}>
                    <Box className={styles.textfield_cardcontent_select_experiment}>
                        <FormControl sx={{ m: 1, minWidth: 120 }} fullWidth={true} error={error.length > 0}>
                            ...
                            >
                                {
                                    experimentIdsInDropdown.map((exp_id)=> 
                                        <MenuItem value={exp_id}>{exp_id}</MenuItem>
                                    )
                                }
                            </Select>
                            {
