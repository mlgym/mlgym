...
class NestedCV(ValidatorIF):
    def __init__(self, dataset_iterator: DatasetIteratorIF, num_outer_loop_folds: int,
                 ...
                 target_pos: int, shuffle: bool, seed: int):
    ...
    def _get_fold_indices(self) -> Tuple[List[DatasetIteratorIF], List[List[DatasetIteratorIF]]]:
    ...
    def _create_blue_prints(self, blue_print_type: Type[BluePrint], gym: Gym, gs_config: Dict[str, Any], num_epochs: int, dashify_logging_path: str):
        ...
        for outer_fold_id, folds in enumerate(inner_folds_indices):
            for inner_fold_id, test_fold_indices in enumerate(folds):
                ...
                external_injection = {
                    ...
                    "id_fold_tags": f"{outer_fold_id}.{inner_fold_id}"
                }
