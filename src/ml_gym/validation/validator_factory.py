...
class ValidatorFactory:
    ...
    def get_nested_cv(gs_config: Dict[str, Any], cv_config: Dict[str, Any], grid_search_id: str,
                      blue_print_type: Type[BluePrint], run_mode: RunMode, keep_interim_results: bool = True) -> NestedCV:
        ...
        return NestedCV(dataset_iterator=iterator, grid_search_id=grid_search_id, **cv_config["NestedCV"]["config"], run_mode=run_mode,
    ...
    def get_cross_validator(gs_config: Dict[str, Any], cv_config: Dict[str, Any], grid_search_id: str,
                            blue_print_type: Type[BluePrint], run_mode: RunMode, keep_interim_results: bool = True) -> CrossValidation:
        iterator_key = cv_config["CV"]["iterator_key"]
        split_key = cv_config["CV"]["split_key"]
        component_names = [iterator_key]
        components = blue_print_type.construct_components(config=gs_config, component_names=component_names)
        iterator = components[iterator_key][split_key]
        return CrossValidation(dataset_iterator=iterator, grid_search_id=grid_search_id, **cv_config["CV"]["config"], run_mode=run_mode,
                               keep_interim_results=keep_interim_results)

    @staticmethod
    def get_gs_validator(grid_search_id: str, run_mode: RunMode, keep_interim_results: bool = True) -> GridSearchValidator:
        return GridSearchValidator(grid_search_id=grid_search_id, run_mode=run_mode, keep_interim_results=keep_interim_results)
