...
class AccelerateGymJob(AbstractGymJob):
    ...
    def __init__(self, experiment_status_logger: ExperimentStatusLogger, gs_api_client: GridSearchAPIClientIF,
        ...
        self.model, self.optimizer, self.trainer, self.evaluator, self.lr_scheduler = self. accelerator.prepare(self.model,
                                                                                                                self.optimizer,
                                                                                                                self.trainer,
                                                                                                                self.evaluator,
                                                                                                                self.lr_scheduler)
        self.trainer.set_accelerator(self.accelerator)
    ...
    def _evaluation_step(self) -> List[EvaluationBatchResult]:
        partial_batch_processed_callback = partial(self.batch_processed_callback, num_epochs=num_epochs,
    ...
    def _execute_train(self):
        ...
        model = self.trainer.train(num_epochs=self.num_epochs, model=self.model, optimizer=self.optimizer,
                                   ...
                                   epoch_done_callback=self.train_epoch_done_callback, accelerator=self.accelerator,
